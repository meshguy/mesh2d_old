...
try                                                   
   ...
   if options.output
     s_msg = 'Checking Geometry';
     if ( options.fid>0 )
       fprintf(options.fid,[s_msg,'\n'])
     else
       gui_Message( s_msg, 2, [1 0] )
     end
   end
...
tic
[qtree.p,qtree.t,qtree.h] = quadtree(node,edge,hdata,options.dhmax,options.output,options.fid);
t_quad = toc;

% Discretise edges
pbnd = boundarynodes(qtree.p,qtree.t,qtree.h,node,edge,options.output,options.fid);
...
if ( options.output )
  c_msg = fieldnames(stats);
  if ( exist('OCTAVE_VERSION')~=0  )
    s_msg = '';
  else
    s_msg = evalc(['disp(stats)']);
  end
  ...
  if ( options.fid>0 )
    ...
    c_msg = regexp(s_msg, '\n','split');
    c_msg(cellfun('isempty',c_msg)) = [];
    gui_Message( c_msg, 2, [0 1] )
  end
end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function p = boundarynodes(ph,th,hh,node,edge,output,fid)
...
if ~isempty(options)
   ...
   if isfield(options,'output')                                            % Output on/off
      ...
      checkposscalar(options.output,'options.output');
   else
      options.output = d_output;
   end
   if ~isfield(options,'fid')                                              % Output to file
      options.fid = 1;
   end
else                                                                       % Default values
   options.mlim   = d_mlim;
