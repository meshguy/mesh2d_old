...
for iter = 1:options.maxit
   ...
   if options.output
     s_msg = sprintf('%2i           %2.1f',[iter, 100.0*min(1.0, options.mlim/max(move,eps))]);
     if ( options.fid>0 )
       fprintf(options.fid,[s_msg,'\n'])
     else
       gui_Message( s_msg, 2 )
   ...
   if done && (max(r)<3.0)                                                 % Main loop convergence
      ...
      if (iter==options.maxit)
         fprintf(options.fid,'WARNING: Maximum number of iterations reached. Solution did not converge! \n');
